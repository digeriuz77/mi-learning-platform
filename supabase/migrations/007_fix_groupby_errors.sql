-- Migration: Fix Practice Analytics Functions (v4)
-- Date: 2026-02-07
-- Description: Fix GROUP BY errors by removing aggregation and using DISTINCT

-- ============================================
-- Fix: Get all users with practice analytics
-- Use DISTINCT instead of GROUP BY to avoid aggregation errors
-- ============================================
DROP FUNCTION IF EXISTS get_all_users_with_practice_analytics(text, int, int);
...
BEGIN
    IF search_email IS NOT NULL THEN
        SELECT json_agg(user_data)::json INTO result
        FROM (
...
-- ============================================
DROP FUNCTION IF EXISTS get_practice_leaderboard(int);
...
DECLARE
    result json;
BEGIN
    SELECT json_agg(leaderboard_data)::json INTO result
    FROM (
        SELECT 
            json_build_object(
                'user_id', ups.user_id,
...
-- ============================================
-- Comprehensive analytics - already working
-- ============================================
DROP FUNCTION IF EXISTS get_comprehensive_practice_analytics();
